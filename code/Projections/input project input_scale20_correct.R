...
results = list()
	...
	for (i in 1:length(GridList)) { 
	  ...
	  mult <- multData$Scale20[multData$GRID==GridList[i]]
        for(j in 1:length(tacs)){
          repf = paste0("Report-scale20-c",tacs[j],".sso") 
          compf = paste0("CompReport-scale20-c",tacs[j],".sso") 		  
...
save(results,file="Grid.MVLN_scale20_correct.RData")
...
#-------------------------------------------
load(file="Grid.MVLN_scale20_correct.RData")
my.path= file.path("projections",'projectionMVN_scale20_correct')
dir.create(my.path)
assessment = "yft_scale20"
...
# 			         type = c(results[['gridIO']]$type,results[['gridSpatio']]$type))
 ...
 Grid = data.frame(model = c(results[['scale20']]$model),
 					 yr = c(results[['scale20']]$yr),
 			      stock = c(results[['scale20']]$stock),
 			     harvest = c(results[['scale20']]$harvest),
 			         tac = c(results[['scale20']]$tac),
 			    tac.perc = c(results[['scale20']]$tac.perc),
 			         type = c(results[['scale20']]$type))
