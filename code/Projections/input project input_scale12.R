...
results = list()
	...
	for (i in 1:length(GridList)) { 
	  multData <- read.csv("output/tables/StockStatus_GRID.csv")
	  mult <- multData$Scale12[multData$GRID==GridList[i]]
        for(j in 1:length(tacs)){
          repf = paste0("Report-scale-c",tacs[j],".sso") 
          compf = paste0("CompReport-scale-c",tacs[j],".sso") 		  
          ...
          Grid = rbind(Grid,data.frame(model=GridList[i],yr=yrs,stock=(y[y$Label %in% paste0(paste0(pars[1],"_"),fyrs),2])/mult,harvest=(y[y$Label %in% paste0(paste0(pars[2],"_"),fyrs),2]),tac=yprj$Value[1],tac.perc=tacs[j],type=ifelse(yrs %in% prjyr,"prj","fit"))) 
...
save(results,file="Grid.MVLN_scale12.RData")
...
#-------------------------------------------
load(file="Grid.MVLN_scale12.RData")
my.path= file.path("projections",'projectionMVN_scale12')
dir.create(my.path)
assessment = "yft_Scale12"
...
# 			         type = c(results[['gridIO']]$type,results[['gridSpatio']]$type))
 ...
 Grid = data.frame(model = c(results[['Scale12']]$model),
 					 yr = c(results[['Scale12']]$yr),
 			      stock = c(results[['Scale12']]$stock),
 			     harvest = c(results[['Scale12']]$harvest),
 			         tac = c(results[['Scale12']]$tac),
 			    tac.perc = c(results[['Scale12']]$tac.perc),
 			         type = c(results[['Scale12']]$type))
...
png(file = paste0("projectionMVN_scale12","/ProjFLR_",assessment,".png"), width = 5, height = 5, 
    res = 200, units = "in")
par(Par)
# Stock
