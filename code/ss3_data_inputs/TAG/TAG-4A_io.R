...
work = data %>% 
  dplyr::filter(project=='RTTP')  %>%
  mutate(rel_age = replace(rel_age,which(rel_age>15),15)) 
write.csv(work, file = file.path(shrpoint_path, 'data/processed', 'tag-filtered-release.csv'), row.names = FALSE)
...
# Save:
write.csv(out_rel, file = file.path(shrpoint_path, 'data/ss3_inputs', spat_config, 'tag-release.csv'), row.names = FALSE)

# Produce recapture SS3 input
workC = dataC %>% 
  dplyr::filter(project=='RTTP')  %>%
  mutate(rel_age = replace(rel_age,which(rel_age>15),15))
write.csv(workC, file = file.path(shrpoint_path, 'data/processed', 'tag-filtered-recapture.csv'), row.names = FALSE)
...
out_rec = workC %>% select(tag,rec_yr,season,rec_model_fleet,number_prime)
# Save:
write.csv(out_rec, file = file.path(shrpoint_path, 'data/ss3_inputs', spat_config, 'tag-recapture.csv'), row.names = FALSE)
