...
# SSB time series:
ts_1A = rbind(
          mod1Aaaf_1$timeseries %>% dplyr::filter(Era == 'TIME') %>% select(Area, Yr, SpawnBio) %>% mutate(dat = 'base', type = 'aaf'),
          mod1Aaaf_2$timeseries %>% dplyr::filter(Era == 'TIME') %>% select(Area, Yr, SpawnBio) %>% mutate(dat = 'addTag', type = 'aaf'),
          mod1Aaaf_3$timeseries %>% dplyr::filter(Era == 'TIME') %>% select(Area, Yr, SpawnBio) %>% mutate(dat = 'addCAAL', type = 'aaf'),
          mod1Aagg_1$timeseries %>% dplyr::filter(Era == 'TIME') %>% select(Area, Yr, SpawnBio) %>% mutate(dat = 'base', type = 'agg'),
          mod1Aagg_2$timeseries %>% dplyr::filter(Era == 'TIME') %>% select(Area, Yr, SpawnBio) %>% mutate(dat = 'addTag', type = 'agg'),
          mod1Aagg_3$timeseries %>% dplyr::filter(Era == 'TIME') %>% select(Area, Yr, SpawnBio) %>% mutate(dat = 'addCAAL', type = 'agg')
)
ts_4A = mod4A$timeseries %>% dplyr::filter(Era == 'TIME') %>% select(Area, Yr, SpawnBio)
...
plot_data_4A = ts_4A %>% group_by(Yr) %>% summarise(SpawnBio = sum(SpawnBio)) %>% mutate(Year = ssts2yq(Yr))
...
p1 = ggplot(plot_data, aes(x = Year, y = SpawnBio, color = factor(dat))) +
        ...
        facet_wrap(~ type)
ggsave(file.path(shrpoint_path, plot_dir, paste0('ts_SSB_1A', img_type)), plot = p1,
...
p2 = ggplot(plot_data, aes(x = Year, y = B.Bmsy, color = factor(dat))) +
  ...
  guides(color = guide_legend(title = 'Data types in model:')) +
  facet_wrap(~ type)
ggsave(file.path(shrpoint_path, plot_dir, paste0('ts_status_1A', img_type)), plot = p2,
       width = img_width, height = 110, units = 'mm', dpi = img_res)
...
p3 = ggplot(plot_data, aes(x = Year, y = dev, color = dat)) +
  ...
  coord_cartesian(xlim = c(1950, 2023)) +
  facet_grid(dat ~ type)
ggsave(file.path(shrpoint_path, plot_dir, paste0('ts_recdev_1A', img_type)), plot = p3,
