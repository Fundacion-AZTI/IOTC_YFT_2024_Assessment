...
Only for the size dataset with regular grids, we then assigned the catch (in numbers) that corresponded to every observation in the size data (i.e., year, quarter, grid, fleet, and gear type). We found a perfect match for $\sim$ 76% of cases but there were some size observations without catch. In order to fill in these catch gaps, we followed an imputation procedure with four levels:
...
@fig-caal shows the CAAL information included in the assessment model per fishery. Adults were mostly sampled from the *LL*, *FS*, and *HD* fisheries. Conversely, juveniles were mainly present in the *LS* fishery. The lengths ranged from 18 to 182 cm and all the model ages (i.e., quarters) were sampled. The largest and oldest fish were observed in the *GI*, *LL*, and *FS* fisheries. We assumed no ageing error in the assessment model.
...
```{r}
#| label: fig-comp-mlen
#| fig-cap: "Comparison between the mean length per quarter per fishery group used in the 2021 and in the current (2024) assessment."
#| fig-align: center
